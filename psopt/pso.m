...
if ~exist('options','var') % Set default options
  options = psooptimset(options); %  Better to be here -- Ben's comment
end % if ~exist
...
if strncmpi(options.PopulationType, 'double', 2)
	% 1: uniformly random independent
	% 2: uniformly random dependent (default)
	% 3: equal space
  state = psocreationuniform(options, nvars, 2); % 1, 2, 3 <= @HERE
...
if ~isempty(Aeq) || ~isempty(Aineq) || ~isempty(nonlcon)
  ...
  if strcmpi(options.ConstrBoundary,'reflect')
    ...
    if options.Verbosity > 2
      ...
      msg = sprintf('%s is not supported for linear constraints.', msg) ;
      msg = sprintf('%s Switching to ''penalize'' method.',msg) ;
      warning('pso:mainfcn:constraintbounds', '%s', msg)
    end
  end
...
% @Ben Initialization function 											@HERE
% ---------------------------------------------------------------------------
if ~isempty(options.InitFcn)
  state = options.InitFcn(state, options, flag);
end % if ~isempty
% ---------------------------------------------------------------------------
...
for iGen = 1:itr
  ...
  if strcmpi(options.ConstrBoundary, 'penalize')  % EXPERIMENTAL
    if strcmpi(options.Vectorized, 'on')          % Vectorized fitness function
      state.Score = fitnessfcn(state.Population, state);              % @HERE
    elseif strcmpi(options.UseParallel,'always')  % Parallel computing
      scoretmp = inf*ones(n,1) ;
      x = state.Population;
      parfor i = 1:n
        scoretmp(i) = fitnessfcn(x(i,:), state);                      % @HERE
      end % for i
      state.Score = scoretmp ;
      clear scoretmp x;
    else % Default
      for i = 1:n
        state.Score(i) = fitnessfcn(state.Population(i,:), state);    % @HERE
      end % for i
    end % if strcmpi
    ...
    if strcmpi(options.Vectorized,'on')  % Vectorized fitness function
      state.Score(not(state.OutOfBounds)) = ...
          fitnessfcn(state.Population(not(state.OutOfBounds),:), state); % @HERE
    elseif strcmpi(options.UseParallel,'always') % Parallel computing
      ...
      for i = find(not(state.OutOfBounds))'
        ...
        state.Score(i) = fitnessfcn(x, state);                   % @HERE
      end % for i
    end % if strcmpi
  end
  % -------------------------------------------------------------------------
	...
	state.xLocalBests(betterindex,1:nvars) = state.Population(betterindex,:); % @HERE
	% -------------------------------------------------------------------------
...
%         ((1/n)*sum(state.Velocities*state.Velocities')).^2) ;
%     tempchk = alpha <= 1.6 ;
  if iGen == 1 || minfitness < state.fGlobalBest(iGen-1)
    stalltime = toc ;
    state.fGlobalBest(iGen) = minfitness ;
    ...
    if imprvchk
      exitflag = 1 ;
      flag = 'done' ;
    elseif state.fGlobalBest(iGen) < options.FitnessLimit
      exitflag = 2 ;
      flag = 'done' ;
    end % if iGen
  ...
  % -------------------------------------------------------------------------
	if ~isempty(options.PostIterateFcn)
		state = options.PostIterateFcn(state, options, flag);
	end % if ~isempty
  % -------------------------------------------------------------------------

  % Check stop criteria
	% -------------------------------------------------------------------------
  stallchk = iGen - state.LastImprovement >= options.StallGenLimit ;
  if stallchk
  ...
  % Update flags, state and plots before updating positions
	% -------------------------------------------------------------------------
	if iGen == 2, flag = 'iter'; end
	if iGen == itr
		flag = 'done';
		exitflag = 0;
	end;

  % Visualise the iteration if specified
	% -------------------------------------------------------------------------
  if ~isempty(options.PlotFcns) && ~mod(iGen, options.PlotInterval)
    ...
    if iscell(options.PlotFcns)
      ...
      for i = 1:length(options.PlotFcns)
        ...
        options.PlotFcns{i}(state, options, flag) ;
      end % for i
    else
  ...
  % -------------------------------------------------------------------------
	state = options.AccelerationFcn(state, options, flag);
  % -------------------------------------------------------------------------
	averagetime = toc/iGen ;
end % for iGen

% Assign output variables and generate output
% ---------------------------------------------------------------------------
xOpt = state.xGlobalBest;
fval = state.fGlobalBest(iGen); % Best fitness value
% Final population: (hopefully very close to each other)
population = state.Population;
scores = state.Score;           % Final scores (NOT local bests)
output.generations = iGen;      % Number of iterations performed

% finalization                                                          @HERE
% ---------------------------------------------------------------------------
if ~isempty(options.FinalFcn)
  output = options.FinalFcn(state, options, output);
end % if ~isempty
% ---------------------------------------------------------------------------
